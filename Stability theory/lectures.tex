...
\textbf{Завдання: } дослідити на стійкість розв'язок $\overline{x} = \overline{0}$ системи (5). Оскільки $\overline{f} \in C^1(D)$, то в деякому околі $Br(\overline{0})$ функцію $\overline{f}$ можно подати у вигляді:
\begin{equation*}
...
Тоді $\forall t \in [t_0, t_1]:$ $$ u(t) \leq (c + b(t-t_0))e^{\mathop{\mathlarger{\int}}\limits_{t_0}^{t} a(s)\mathrm{d}s}$$
\end{boxlema}

\textbf{Доведення. } Розглянемо допоміжну функцію: $v(t) = c  +  b(t-t_0) + \mathop{\mathlarger{\int}}\limits_{t_0}^{t}a(s)u(s)\mathrm{d}s$ Тоді: $$ v(t_0) = c; \quad u(t) \leq v(t) \quad \forall t \in [t_0, t_1]; \quad v'(t) = b + a(t)u(t) \leq b + a(t)v(t)$$ Отже, $v'(t) \leq b + a(t)v(t)$, домножимо ліву і праву частину на $e^{-\mathop{\mathlarger{\int}}\limits_{t_0}^{t} a(s)\mathrm{d}s}$, тоді:
