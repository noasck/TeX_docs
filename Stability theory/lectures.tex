...
Отже, $\forall t_0 \geq  a \quad \forall \varepsilon >0 \quad \exists \delta > 0 \quad \left(  \delta = \dfrac{\varepsilon }{K || X^{-1} (t_0)|| + 1}  \right)$  для довільного розв'язку з $ || \overline{x}_0|| < \delta$ справедливо $ ||\overline{x} (t)|| < \varepsilon  \quad \Longrightarrow \quad $ стійкість розв'язку.\\

\textbf{Доведення.} a) \fbox{$\Longrightarrow$} \\ Нехай (3) - стійка. Припустимо від супротивного, що $\exists  \left\lbrace t_n \right\rbrace_{n\geq 1}^{\infty} : ||X(t_n)|| \to {+\infty} $ при $ n \to \infty$.\\
