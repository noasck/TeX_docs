...
\newcommand\prefix@section{Лекція \thesection }
\makeatother
...
\end{defo}
Нехай $\ty(x)$ -- розв`язок задачі зі старшими похідними. Тоді т.$\alpha = 0 $ є точкою локального екстремуму функції $\Phi (\alpha)$ . Отже $\Phi' (0) = 0 \Longrightarrow$ \fbox{$\delta J(\ty, h)=0$} для кожного допустимого приросту $h$. Таким чином, отримали необхідну умову розв'язку в термінах першої варіації функціоналу.
...
\defo Другою варіацією функціоналу J(y) на функції $y(x) \in M$ в нарямку $h$, де $h(x) \in C^[a, b] : h(a) = h(b) = 0$, називається: $$\delta^2 J(y, h) = \dfrac{\mathrm{d}^2}{\mathrm{d \alpha^2}} J(y + \alpha h) \Big|_{\alpha = 0} \ (= \Phi''(0)) $$
